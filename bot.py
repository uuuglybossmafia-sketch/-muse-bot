...
def ask_openai(system, history):
    ...
    if "choices" not in response:
        error_msg = response.get("error", {}).get("message", "Unknown OpenAI error")
...
path = Path('/mnt/data/fixed_bot.py')
path.write_text(corrected_code, encoding='utf-8')

print(f"Saved corrected bot to: {path}")
